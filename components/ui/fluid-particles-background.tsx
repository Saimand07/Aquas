...
export const FluidParticlesBackground = ({
  ...
  useEffect(() => {
    ...
    const animate = () => {
      const isDark = true; // Always dark luxury aesthetic for Midnight Aquas
      const scheme = isDark ? COLOR_SCHEME.dark : COLOR_SCHEME.light;

      // Clear canvas with subtle background to create smooth flowing trails
      ...
      for (const particle of particles) {
        ...
        ctx.fillStyle = `rgba(176, 141, 87, ${opacity * 0.6 + 0.05})`; // Gold/brass tint
        ctx.beginPath();
  ...
  if (isFixed) {
    return (
      <div className={cn("fixed inset-0 pointer-events-none z-0 overflow-hidden", className)}>
        <canvas ref={canvasRef} className={cn("absolute inset-0 w-full h-full", canvasClassName)} />
      </div>
    );
  }
  ...
  return (
    ...
    >
      <canvas ref={canvasRef} className={cn("fixed inset-0 w-full h-full pointer-events-none z-0", canvasClassName)} />
